...
//set the pre function
const char *PRE = "044553312666000000";	
			...
			if(regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
				...
				switch(rules[i].token_type) {
					...
					case HEX:case DEC:case REG:case MARK:
						...
						tokens[nr_token].str[substr_len] = '\0';		//add '\0'
					default:
...
uint32_t eval(int l, int r, bool *success) {
	*success = true;
	if(l > r)  return *success = false;// invalid
	if(l == r){				
		uint32_t t;
		if(tokens[l].type == HEX) {
			sscanf(tokens[l].str, "%x", &t);
			return t;
		} else if(tokens[l].type == DEC) {
			sscanf(tokens[l].str, "%d", &t);
			return t;
