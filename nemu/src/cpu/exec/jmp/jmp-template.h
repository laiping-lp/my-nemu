...
static void do_execute () {
    ...
    if(op_src->type == OP_TYPE_REG || op_src->type == OP_TYPE_MEM){
        cpu.eip = temp - concat(decode_rm_, SUFFIX)(cpu.eip+1) - 1;
        print_asm_template1();
    }
    else{
        print_asm("jmp %x", cpu.eip + 1 + DATA_BYTE + temp);
        cpu.eip += temp;
    }
}
...
// #if DATA_BYTE == 4
// make_helper(ljmp){
//     exturn SEG_descriptor *seg_des;
//     SEG_descriptor seg;
//     seg_des = &seg;
//     unit32_t op1 = instr_fetch(eip+1, 4) - 7;
//     unit16_t op2 = instr_fetch(eip+5, 2);
//     cpu.eip = op1;
//     cpu.cs.selector = op2;
